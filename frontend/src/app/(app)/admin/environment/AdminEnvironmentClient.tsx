...
import { useTheme, type DarkVariant, type LightVariant } from '@/components/providers/ThemeProvider'
...
// ──────────────────────────────────────────────────────────────────
// Alpha themes section — opt-in experimental visual variants.
// ──────────────────────────────────────────────────────────────────
type AlphaLightOption = { id: Exclude<LightVariant, 'default'>; name: string; primary: string; secondary: string; bg: string; tagline: string }
type AlphaDarkOption  = { id: 'core-interface' | 'platform-architecture'; name: string; primary: string; secondary: string; bg: string; tagline: string }

const ALPHA_LIGHT_OPTIONS: AlphaLightOption[] = [
  {
    id: 'nova-estate',
    name: 'Nova Estate',
    primary: '#14B8A6',
    secondary: '#DBEAFE',
    bg: '#FFFFFF',
    tagline: 'Glassmorphism · teal accent · 24px cards',
  },
]
const ALPHA_DARK_OPTIONS: AlphaDarkOption[] = [
  {
    id: 'core-interface',
    name: 'Core Interface',
    primary: '#22D3EE',
    secondary: '#06B6D4',
    bg: '#0A0A0C',
    tagline: 'Electric cyan · elevated cards · dot-matrix bg',
  },
  {
    ...
    secondary: '#54B6AB',
    bg: '#0A2220',
    tagline: 'Outlined surfaces · 2px radii · minimal motion',
  },
]

function AlphaThemesSection() {
  const {
    alphaThemesEnabled, setAlphaThemesEnabled,
  ...
  const onToggle = (next: boolean) => {
    setAlphaThemesEnabled(next)
    if (next) {
      // Pick sensible defaults the first time alpha is turned on.
      if (lightVariant === 'default') setLightVariant('nova-estate')
      if (darkVariant !== 'core-interface' && darkVariant !== 'platform-architecture') {
        setDarkVariant('core-interface')
      }
    } else {
      // Don't wipe the user's saved choice — the provider will fall back
      // to the production palette automatically while the toggle is off.
    }
  }
  ...
  return (
    <section className="rounded-md border p-3 bg-[hsl(var(--card))]">
      <div className="flex items-start justify-between gap-3 mb-2">
        <div>
          <h3 className="text-sm font-semibold flex items-center gap-2">
            Alpha themes
            <span className="inline-flex items-center px-1.5 py-0.5 rounded font-mono text-[9px] tracking-wider uppercase bg-[hsl(var(--accent)/0.18)] text-[hsl(var(--primary-deep))] border border-[hsl(var(--primary)/0.25)] dark:bg-[hsl(var(--primary)/0.14)] dark:text-[hsl(var(--primary))] dark:border-[hsl(var(--primary)/0.32)]">experimental</span>
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Try out new visual designs without committing to them. Toggle off any time to return to the production look.
            Currently active mode: <span className="font-medium text-foreground">{resolved}</span>.
          </p>
        </div>
        <label className="inline-flex items-center gap-2 select-none cursor-pointer shrink-0">
          <input
            type="checkbox"
            className="h-4 w-4 accent-[hsl(var(--primary))]"
            checked={alphaThemesEnabled}
            onChange={(e) => onToggle(e.target.checked)}
          />
          <span className="text-sm font-medium">{alphaThemesEnabled ? 'Enabled' : 'Disabled'}</span>
        </label>
      </div>

      <div className={alphaThemesEnabled ? '' : 'opacity-60 pointer-events-none'}>
        <div className="mt-3">
          <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Light mode</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <AlphaPaletteCard
              active={lightVariant === 'default'}
              onPickAction={() => setLightVariant('default')}
              name="Bayan default"
              tagline="Production cyan look"
              primary="#29B5E8"
              secondary="#7DD3F3"
              bg="#FFFFFF"
              isDark={false}
            />
            {ALPHA_LIGHT_OPTIONS.map(opt => (
              <AlphaPaletteCard
                key={opt.id}
                active={lightVariant === opt.id}
                onPickAction={() => setLightVariant(opt.id)}
                name={opt.name}
                tagline={opt.tagline}
                primary={opt.primary}
        ...
        <div className="mt-4">
          <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Dark mode</div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <AlphaPaletteCard
              active={darkVariant === 'bluish' || darkVariant === 'blackish'}
              onPickAction={() => setDarkVariant('bluish')}
              name="Bayan default"
              tagline="Production deep-cyan look"
              primary="#29B5E8"
              secondary="#7DD3F3"
              bg="#011419"
              isDark
            />
            {ALPHA_DARK_OPTIONS.map(opt => (
              <AlphaPaletteCard
                key={opt.id}
                active={darkVariant === opt.id}
                onPickAction={() => setDarkVariant(opt.id)}
                name={opt.name}
                tagline={opt.tagline}
                primary={opt.primary}
                secondary={opt.secondary}
      ...
      <p className="text-[11px] text-muted-foreground mt-3 leading-snug">
        Choices are saved to your browser. The toggle is a master gate: with it off, alpha selections are remembered
        but not applied. Some components use hardcoded colors that won't switch — those are flagged in the rollout
        plan and will be migrated as variants harden.
      </p>
    </section>
  )
}

function AlphaPaletteCard({
  active, onPickAction, name, tagline, primary, secondary, bg, isDark,
}: {
  active: boolean
  onPickAction: () => void
  name: string
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={`text-left rounded-md border p-2 cursor-pointer transition-colors duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--primary))] ${active
        ? 'border-[hsl(var(--primary))] bg-[hsl(var(--accent)/0.18)] dark:bg-[hsl(var(--primary)/0.14)]'
        : 'border-[hsl(var(--border))] bg-[hsl(var(--background))] hover:bg-[hsl(var(--muted))]'}`}
    >
      <div className="flex items-center gap-2">
        {/* Mini chip preview */}
        <div
          className="w-12 h-12 rounded-md shrink-0 border border-black/10 dark:border-white/10 overflow-hidden"
          style={{ background: bg }}
          aria-hidden
        >
          <div className="h-1/2 w-full" style={{ background: primary }} />
          <div className="h-1/2 w-full" style={{ background: secondary }} />
        </div>
        <div className="min-w-0">
          <div className="text-sm font-medium truncate flex items-center gap-1.5">
            {name}
            {active && <span className="text-[9px] px-1 rounded bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] font-mono uppercase tracking-wider">on</span>}
          </div>
          <div className="text-[11px] text-muted-foreground truncate">{tagline}</div>
          <div className="text-[10px] text-muted-foreground/70 font-mono mt-0.5">{primary} · {isDark ? 'dark' : 'light'}</div>
        </div>
      </div>
    </button>
  )
