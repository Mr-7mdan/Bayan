...
def _resolve_join_catalog(ds_transforms: dict) -> dict:
    """Upgrade 2-part join targetTable values (schema.table) to 3-part (catalog.schema.table)
    by querying DuckDB's information_schema. This is needed for MySQL-attached catalogs where
    the joins are stored as mt5.mt5_deals_lp but DuckDB requires pcma.mt5.mt5_deals_lp."""
    if not ds_transforms or _duckdb is None:
        return ds_transforms
    joins = (ds_transforms or {}).get("joins") or []
    if not joins:
        return ds_transforms
    try:
        with open_duck_native(get_active_duck_path()) as _jconn:
            for _jj in joins:
                _jt = str((_jj or {}).get("targetTable") or "").strip()
                _jt_parts = _jt.split(".")
                if len(_jt_parts) == 2:
                    try:
                        _jrow = _jconn.execute(
                            "SELECT table_catalog FROM information_schema.tables "
                            "WHERE table_schema = ? AND table_name = ? "
                            "AND table_catalog <> current_catalog() LIMIT 1",
                            (_jt_parts[0], _jt_parts[1])
                        ).fetchone()
                        if _jrow:
...
# --- Helpers ---
# Pure result-shaping helpers extracted to app/query_shaping.py (spec 11, Phase A).
# Re-imported here so existing bare-name call sites keep working unchanged.
from ..query_shaping import _http_for_db_error, _coerce_date_like, _json_safe_cell
...
def _resolve_table_name(ds: Any, source_table_id: str | None, source_name: str | None) -> str | None:
    ...
    # Extract remote attachments for DuckDB ATTACH replay
...
def run_query(payload: QueryRequest, db: Session = Depends(get_db), actorId: Optional[str] = None, publicId: Optional[str] = None, token: Optional[str] = None) -> QueryResponse:
    ...
    try:
        ...
        ds_obj = None
        ds_type_lower = ""
    _tbl_part_count = len((tbl or '').split('.'))
    _duck_check = _duck_has_table(tbl)
        ...
        try:
            ...
            if payload.datasourceId and ds_obj:
                try:
                    ...
                    _remote_attachments = []
            # Fallback: if no attachments from the payload datasource but the SQL
            # contains catalog-qualified (3-part) names, scan all DuckDB datasources
            # for remoteAttachments so the ATTACH happens even on the first request.
            if not _remote_attachments and '.' in (payload.sql or ''):
                _has_3part = bool(re.search(
                    r'"[^"]+"\s*\.\s*"[^"]+"\s*\.\s*"[^"]+"', payload.sql or ''
                )) or bool(re.search(
                    r'\b\w+\.\w+\.\w+', payload.sql or ''
                ))
                if _has_3part:
                    try:
                        _all_ds = db.query(Datasource).filter(
                            Datasource.type.ilike('%duckdb%')
                        ).all()
                        for _ds_item in _all_ds:
                            try:
                                _opts = json.loads(getattr(_ds_item, 'options_json', None) or '{}')
                                _atts = (_opts.get('transforms') or {}).get('remoteAttachments') or []
                                if _atts:
                                    _remote_attachments.extend(_atts)
                            except Exception:
                                continue
                    except Exception:
                        pass
                    # Auto-generate attachments by matching 3-part catalog prefixes to datasource names.
                    # e.g. "pcma"."mt5"."mt5_deals" → attach datasource named "pcma" as alias "pcma"
                    if not _remote_attachments:
                        try:
                            _cat_prefixes = set()
                            for _m3 in re.finditer(
                                r'"([^"]+)"\s*\.\s*"[^"]+"\s*\.\s*"[^"]+"',
                                payload.sql or ''
                            ):
                                _cat_prefixes.add(_m3.group(1).lower())
                            for _pfx in _cat_prefixes:
                                try:
                                    _pfx_ds = db.query(Datasource).filter(
                                        Datasource.name.ilike(_pfx)
                                    ).first()
                                    if _pfx_ds and (getattr(_pfx_ds, 'type', '') or '').lower() not in ('duckdb',):
                                        _remote_attachments.append({
                                            'alias': _pfx_ds.name,
...
def run_query_spec(payload: QuerySpecRequest, db: Session = Depends(get_db), actorId: Optional[str] = None, publicId: Optional[str] = None, token: Optional[str] = None, _guard: None = Depends(_spec_concurrency_guard)) -> QueryResponse:
    ...
    # NOTE: We attempt this for ALL datasource types — the table may be DuckDB-attached
    # even when the datasource is typed as MySQL/Postgres.
    try:
        _src_parts = str(payload.spec.source or '').split('.')
        if len(_src_parts) == 2 and _duckdb is not None:
    ...
    # instead of MySQL backtick syntax which DuckDB cannot parse.
    # 3-part names (catalog.schema.table) are DuckDB catalog syntax by definition — always duckdb.
    # EXCEPT when the user explicitly selected a non-DuckDB datasource (e.g. MySQL/MSSQL/Postgres).
    # In that case, the 3-part name was an internal CatalogResolve artifact; the query must still
    # go directly to that datasource in its native dialect.
    _spec_src_parts = len((payload.spec.source or '').split('.'))
    ...
    def _apply_scope(ds_tr: dict, source_name: str) -> dict:
        ...
        return {
            'customColumns': filt(ds_tr.get('customColumns')),
            'transforms': filt(ds_tr.get('transforms')),
            'joins': filt_joins(ds_tr.get('joins')),
            'defaults': ds_tr.get('defaults') or {},
        }
                ...
                if val_field.lower() in _pav_computed_lower:
                    ...
                    try:
                        if 'duckdb' in d:
                            ...
                            # Non-DuckDB: probe columns via engine
                            _pav_engine = _engine_for_datasource(db, payload.datasourceId, actorId)
                            with _pav_engine.connect() as _pav_ec:
                                _pav_res0 = _pav_ec.execute(text(f"SELECT * FROM {_q_source(spec.source)} WHERE 1=0"))
                                _pav_cols = set(str(c) for c in _pav_res0.keys())
                    except Exception:
                        pass
    ...
    # ── Moving-average early exit ──────────────────────────────────────────────
    # ma7 / ma14 / ma30 / ma60
    # Algorithm:
    #   1. Find the last date in the filtered window  → max_date
    #   2. Define the MA window: [max_date - (N-1) days, max_date]
    #   3. For each day in that window compute SUM(value)
    #   4. Divide total SUM by N  → the N-day moving average
    #
    # This is equivalent to: SUM(value for last N days) / N
    # which is the standard "trailing MA" interpretation for a scalar KPI.
    _MA_AGG = frozenset({'ma7', 'ma14', 'ma30', 'ma60'})
    if agg in _MA_AGG:
        ...
        # ── Resolve computed val_field for MA path (same as avg period path) ───
        if ds is not None:
            try:
                _mav_opts = json.loads(getattr(ds, 'options_json', None) or '{}')
                _mav_tr = _apply_scope((_mav_opts or {}).get("transforms") or {}, spec.source)
                _mav_computed_lower: dict[str, str] = {}
                for _cc in (_mav_tr.get("customColumns") or []):
                    _n = str((_cc or {}).get("name") or "").strip()
                    if _n: _mav_computed_lower[_n.lower()] = _n
                for _tr in (_mav_tr.get("transforms") or []):
                    _n = str((_tr or {}).get("name") or (_tr or {}).get("target") or "").strip()
                    if _n: _mav_computed_lower[_n.lower()] = _n
                if val_field and val_field.lower() in _mav_computed_lower:
                    val_field = _mav_computed_lower[val_field.lower()]
                    vcol = _q_ident(val_field)
                    _mav_cols: set = set()
                    try:
                        if 'duckdb' in d:
                            with open_duck_native(settings.duckdb_path) as _mav_conn:
                                _mav_cur = _mav_conn.execute(f"SELECT * FROM {_q_source(spec.source)} WHERE 1=0")
                                _mav_desc = getattr(_mav_cur, 'description', None) or []
                                _mav_cols = set(str(c[0]) for c in _mav_desc)
                        else:
                            _mav_engine = _engine_for_datasource(db, payload.datasourceId, actorId)
                            with _mav_engine.connect() as _mav_ec:
                                _mav_res0 = _mav_ec.execute(text(f"SELECT * FROM {_q_source(spec.source)} WHERE 1=0"))
                                _mav_cols = set(str(c) for c in _mav_res0.keys())
                    except Exception:
                        pass
                    _mav_tr_filt = _filter_by_basecols(_mav_tr, _mav_cols) if _mav_cols else _mav_tr
                    _mav_cols_lower = {c.lower() for c in (_mav_cols or set())}
                    _mav_joins = [j for j in (_mav_tr_filt.get('joins') or [])
                                  if not _mav_cols or str((j or {}).get('sourceKey') or '').strip().lower() in _mav_cols_lower]
                    _mav_bres = build_sql(
                        dialect=ds_type,
                        source=_q_source(spec.source),
                        base_select=["*"],
                        custom_columns=_mav_tr_filt.get("customColumns", []),
                        transforms=_mav_tr_filt.get("transforms", []),
                        joins=_mav_joins,
                        defaults={},
                        limit=None,
                        base_cols=_mav_cols,
        ...
        _ma_params: dict = {}
        _ma_where_source = getattr(spec, 'where', None) or {}
        for _wk, _wv in _ma_where_source.items():
            if _wv is None:
                continue
            _wbase = _wk.split('__')[0] if '__' in _wk else _wk
            _wop   = _wk.split('__', 1)[1] if '__' in _wk else 'eq'
            ...
            if isinstance(_wv, list):
                ...
                _ma_where_parts.append(f"{_wcol} IN ({phs})")
                for _i, _v in enumerate(_wv): _ma_params[f'{_pn}_{_i}'] = _v
            elif _wop == 'gte': _ma_where_parts.append(f"{_wcol} >= :{_pn}"); _ma_params[_pn] = _wv
            elif _wop == 'gt':  _ma_where_parts.append(f"{_wcol} >  :{_pn}"); _ma_params[_pn] = _wv
            elif _wop == 'lte': _ma_where_parts.append(f"{_wcol} <= :{_pn}"); _ma_params[_pn] = _wv
            elif _wop == 'lt':  _ma_where_parts.append(f"{_wcol} <  :{_pn}"); _ma_params[_pn] = _wv
            elif _wop == 'ne':  _ma_where_parts.append(f"{_wcol} != :{_pn}"); _ma_params[_pn] = _wv
            else:               _ma_where_parts.append(f"{_wcol} =  :{_pn}"); _ma_params[_pn] = _wv

        _ma_where_clause = f" WHERE {' AND '.join(_ma_where_parts)}" if _ma_where_parts else ""

        # ── Dialect-aware date cast & interval arithmetic ──────────────────────
        if 'mysql' in d:
            _ma_date_expr  = f"DATE({dcol_raw})"
            _ma_interval   = f"INTERVAL {_ma_window - 1} DAY"
            _ma_sub_expr   = f"DATE_SUB((SELECT MAX({_ma_date_expr}) FROM {_ma_from_sql}{_ma_where_clause}), {_ma_interval})"
        elif 'mssql' in d or 'sqlserver' in d:
            _ma_date_expr  = f"CAST({dcol_raw} AS DATE)"
            _ma_sub_expr   = f"DATEADD(day, -{_ma_window - 1}, (SELECT MAX({_ma_date_expr}) FROM {_ma_from_sql}{_ma_where_clause}))"
        elif 'postgres' in d or 'postgre' in d:
            _ma_date_expr  = f"CAST({dcol_raw} AS DATE)"
            _ma_sub_expr   = f"((SELECT MAX({_ma_date_expr}) FROM {_ma_from_sql}{_ma_where_clause}) - INTERVAL '{_ma_window - 1} days')"
        else:  # duckdb / default
            _ma_date_expr  = f"CAST({dcol_raw} AS DATE)"
    ...
    if not has_chart_semantics:
        ...
        _source_part_count = len((spec.source or '').split('.'))
        # 3-part names (catalog.schema.table) are DuckDB catalog syntax — always route to DuckDB.
        # For prefer_local, also verify DuckDB has the table.
        if _source_part_count >= 3:
            _eff_ds_id = None
        elif prefer_local and _duck_has_table(spec.source):
            _eff_ds_id = None
        else:
        ...
        if not x_col and not (spec.legend or legend_orig):
            ...
            if series_scalar and len(series_scalar) > 0:
                ...
                if union_parts:
                    ...
                    q = QueryRequest(
                        sql=sql_inner,
                        datasourceId=(None if ('duckdb' in (ds_type or '')) or (prefer_local and _duck_has_table(spec.source)) else payload.datasourceId),
                        limit=eff_limit,
                        offset=off or 0,
                        includeTotal=payload.includeTotal,
                        params=params or None,
...
def distinct_values(payload: DistinctRequest, request: Request, db: Session = Depends(get_db), actorId: Optional[str] = Depends(actor_id_optional), publicId: Optional[str] = None, token: Optional[str] = None) -> DistinctResponse:
    ...
    # Resolve 2-part source (schema.table) to 3-part (catalog.schema.table) for
    # MySQL-attached catalogs in DuckDB — attempt for ALL datasource types.
    try:
        _dist_src_parts = str(payload.source or '').split('.')
        if len(_dist_src_parts) == 2 and _duckdb is not None:
            with open_duck_native(get_active_duck_path()) as _dconn:
                _drow = _dconn.execute(
                    "SELECT table_catalog FROM information_schema.tables "
                    "WHERE table_schema = ? AND table_name = ? "
                    "AND table_catalog <> current_catalog() LIMIT 1",
                    (_dist_src_parts[0], _dist_src_parts[1])
                ).fetchone()
                if _drow:
    ...
    # CatalogResolve for /distinct — mirror run_query_spec logic exactly.
    # 3-part names are DuckDB catalog syntax, EXCEPT when the user explicitly selected
    # a non-DuckDB datasource (MySQL/MSSQL/Postgres). In that case strip the catalog prefix.
    _dist_src_parts_count = len((payload.source or '').split('.'))
    ...
    route_duck = False
    dialect = "unknown"
    _src_part_count = len((payload.source or '').split('.'))
    _duck_has_src = _duck_has_table(payload.source)
    # 3-part names (e.g. pcma.mt5.mt5_deals) are DuckDB catalog references.
    # Also route to DuckDB when prefer_local is set or source has 3+ parts and DuckDB can serve it.
    if (payload.datasourceId is None) or ((ds_info or {}).get("type", "").lower() == "duckdb") or ((_src_part_count >= 3 or bool(settings.prefer_local_duckdb)) and _duck_has_src):
        route_duck = True
        dialect = "duckdb"
    else:
        engine = _engine_for_datasource(db, payload.datasourceId, actorId)
    ...
    if ds_info is not None:
        ...
        def _list_cols_for_base() -> set[str]:
            try:
                if route_duck or (ds_type or '').lower().startswith('duckdb') or (payload.datasourceId is None):
                    from ..db import open_duck_native
                    with open_duck_native(settings.duckdb_path) as conn:
        ...
        ds_transforms_for_sql = None  # fallback: use ds_transforms directly if try block fails
        try:
        ...
        # Use ds_transforms_for_sql (filtered for this field) not ds_transforms (schema copy)
        _sql_transforms = ds_transforms_for_sql if isinstance(ds_transforms_for_sql, dict) else ds_transforms
        __joins_all = _sql_transforms.get('joins', []) if isinstance(_sql_transforms, dict) else []
        __joins_eff = []
        for __j in (__joins_all or []):
            try:
                __skey = str((__j or {}).get('sourceKey') or '').strip()
        ...
        if route_duck and _duckdb is not None:
            ...
            if datasource_id and ds and getattr(ds, "connection_encrypted", None):
                try:
                    ...
                    pass
            with open_duck_native(db_path) as conn:
                _apply_duck_mysql_attachments(conn, _pt_remote_attachments, db)
                try:
                    _replay_attaches_on_conn(conn)
                except Exception:
                    pass
